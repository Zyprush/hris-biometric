...
const Modal: React.FC<ModalProps> = ({
  ...
  const [editedEmployee, setEditedEmployee] = useState<EmployeeDetails | null>(
    null
  );
  ...
  const [tempProfilePicUrl, setTempProfilePicUrl] = useState<string | null>(
    null
  );
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to delete ${employee.name}'s record?`
      )
    ) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "employment":
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <DetailItem
              label="sss"
              value={editedEmployee.sss}
              isEditing={isEditing}
              onChange={handleInputChange}
            />
            <DetailItem
              label="philHealthNumber"
              value={editedEmployee.philHealthNumber}
              isEditing={isEditing}
              onChange={handleInputChange}
            />
            <DetailItem
              label="pagIbigNumber"
              value={editedEmployee.pagIbigNumber}
              isEditing={isEditing}
              onChange={handleInputChange}
            />
            <DetailItem
              label="tinNumber"
              value={editedEmployee.tinNumber}
              isEditing={isEditing}
              onChange={handleInputChange}
            />
          </div>
      ...
      case "documents":
        return (
          <div>
            {editedEmployee.documentUrls &&
            editedEmployee.documentUrls.length > 0 ? (
                  ...
                  {isEditing && (
                    <div className="mt-2">
                      <label
                        ...
                        className="cursor-pointer bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition-colors duration-200"
                      >
                        Choose New Picture
                      </label>
                      <input
...
const DocumentListItem: React.FC<{ url: string }> = ({ url }) => {
  ...
  const fileName =
    decodedUrl.split("/").pop()?.split("?")[0] || "Unknown Document";
  return (
