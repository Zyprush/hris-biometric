...
const NavTitle: React.FC<TextSettingProps> = ({
  name,
  title,
}): JSX.Element => {
  ...
  return (
    <div className="flex flex-col items-center justify-apart w-auto ml-0 mr-auto group text-zinc-600">
      {loading ? (
        <p>Loading...</p>
      ) : (
        <>
          {isEditing ? (
            <div className="flex justify-between items-center">
              <input
                ...
                className="input input-sm border mx-auto w-80"
            ...
            <div className="flex w-full my-auto justify-between items-center group">
              <p className="text-xl text-primary font-bold ml-1 max-w-80 text-center">
                {value ? value : `No data for ${title}`}
              </p>
              <button
                onClick={toggleEdit}
                className="btn btn-outline btn-sm rounded-sm text-primary mx-auto hover:text-secondary ml-auto mr-0 hidden group-hover:flex"
              >
                Edit
              </button>
            </div>
